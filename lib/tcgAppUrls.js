...
/**
 * Login: force Indonesia (+62) in Express. Path `/id/login` may redirect to `/login` (SG) on the app;
 * `?locale=id` is the usual way to keep ID context on `/login`. Override via NEXT_PUBLIC_TCG_LOGIN_URL if needed.
 */
const DEFAULT_TCG_LOGIN_URL = `${TCG_APP_ORIGIN}/login?locale=id`;
