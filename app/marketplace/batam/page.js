...
export default function BatamPage() {
  ...
  const corridorRows = [
    { icon: '🚢', label: 'Ferry harian', value: 'Harbourfront ↔ Batam Centre / Sekupang / Nongsapura' },
    { icon: '🌐', label: 'FTZ', value: 'Bebas bea masuk, PPN, dan PPnBM' },
    { icon: '📦', label: 'TCG Express', value: 'Pengiriman last-mile di SG dan ID' },
    { icon: '⏱️', label: 'Lead time', value: '1–2 hari SG ke Batam (via ferry + last-mile)' },
  ];
  ...
  return (
    <div style={{ background: '#0d0f14', color: '#e8eaf0', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', top: '40%', left: '50%', transform: 'translate(-50%,-50%)', width: '600px', height: '300px', background: 'radial-gradient(ellipse, rgba(62,207,142,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />

        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(62,207,142,0.1)', border: '1px solid rgba(62,207,142,0.25)', borderRadius: '20px', padding: '6px 16px', marginBottom: '24px', fontSize: '13px', color: '#3ecf8e', fontWeight: '600' }}>
          🏝️ Batam Free Trade Zone · Indonesia
        </div>

        <h1 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: '800', margin: '0 0 16px', lineHeight: '1.15', background: 'linear-gradient(135deg, #e8eaf0 0%, #3ecf8e 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
        ...
        <div style={{ display: 'flex', gap: '14px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="/marketplace/rfq?location=Batam" style={{
            textDecoration: 'none', padding: '14px 28px', borderRadius: '10px',
            background: 'linear-gradient(135deg, #3ecf8e, #2ba870)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
          {benefits.map((b, i) => (
            <div key={i} style={{
              ...
              <h3 style={{ margin: '0 0 8px', fontSize: '15px', fontWeight: '700', color: '#e8eaf0' }}>{b.title}</h3>
              <p style={{ margin: 0, fontSize: '13px', color: '#7a8099', lineHeight: '1.5' }}>{b.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 56px', maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ background: '#13161e', border: '1px solid rgba(62,207,142,0.2)', borderRadius: '16px', padding: '32px' }}>
          <h2 style={{ margin: '0 0 24px', fontSize: '18px', fontWeight: '700', color: '#3ecf8e' }}>🗺️ Koridor SG–Batam</h2>
      ...
      <section style={{ padding: '0 24px 56px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: '700', margin: 0, color: '#e8eaf0' }}>Vendor Tersedia di Batam</h2>
          {usingSeed ? (
            <span
              style={{
                fontSize: '11px',
      ...
      <section style={{ background: 'linear-gradient(135deg, #13161e, #1a1e28)', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '48px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: '22px', fontWeight: '800', color: '#e8eaf0', margin: '0 0 10px' }}>Jadilah Driver TCG Express di Batam</h2>
